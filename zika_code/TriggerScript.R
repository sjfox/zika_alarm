...
colnames(county_plot.m) <- c("id", "geography", "R0_round", "importation_probability", "scenario", "import.rate")

### Calculate Surveillance Triggers For R0s and Import Rates 
rnots <- sort(unique(county_plot.m$R0_round))
import.rates <- sort(unique(county_plot.m$import.rate))

triggers <- get_trigger_data(rnots, intro = import.rates, disc = 0.0224, threshold = 20, confidence = .8)
...
for (i in 1:nrow(triggers)) {
  indices = which((county_plot.m$R0_round == triggers[i,"r_not"]) & 
...
final.plot$import.type <- factor(final.plot$scenario, 
                                 levels = c("importation.current", "importation.projected", "importation.worse.projected"))
...
import.breaks <- seq(from = 0, to = .15, by = .015)
plot.importation <- ggplot(final.plot, aes(x = long, y = lat)) +
  geom_polygon(data = final.plot, aes(group = group, fill = importation_probability), color = "black", size = .25) +
  scale_x_continuous("", breaks=NULL) + 
  scale_y_continuous("", breaks=NULL) + 
  scale_fill_continuous(name = "Importation \n Probability \n", low = "light yellow", high = "red", 
